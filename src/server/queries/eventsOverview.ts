...
export async function getEventsOverview(ctx: ServerAuthContext = {}) {
  ...
  const nextEvents = upcomingEvents.slice(0, 8).map(event => {
    ...
    return {
      ...
      allowChildren: Boolean(event.allowChildren),
      childTicketPrice: asNumber(event.childTicketPrice),
      servantsSlug: asText(event.servantsSlug, ''),
      servantsPrice: asNumber(event.servantsPrice),
