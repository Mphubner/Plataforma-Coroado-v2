...
import { Search, Users, Heart, Shield, Edit2, X, Check, MapPin, Network, List } from "lucide-react";
...
export function MembersView({ userData }: { userData?: any }) {
  ...
  const handleUpdateMember = async () => {
    ...
    try {
      await updateDoc(doc(db, 'users', editingMember.id), {
        ...
        isApproved: editingMember.isApproved,
  ...
  const leaders = members.filter(m => m.roles?.includes('leader') || m.roles?.includes('pastor') || m.roles?.includes('admin')).length;

  const renderTree = (parentId?: string, level = 0) => {
    const children = members.filter(m => {
       if (!parentId) return !m.supervisorId && m.roles?.includes('pastor'); // Root nodes (Senior Pastors)
          ...
          <CardContent className="space-y-4">
             ...
             <div className="space-y-2 pt-4">
               ...
               <div className="flex flex-wrap gap-2">
                 {['admin', 'pastor', 'leader', 'volunteer', 'member'].map(role => (
                   <Badge 
                     key={role} 
        ...
        <CardContent>
           <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
             ...
             <TabsContent value="tree" className="m-0 p-4 border border-white/10 rounded-lg bg-black overflow-x-auto min-h-[300px]">
                {members.filter(m => !m.supervisorId && m.roles?.includes('pastor')).length > 0 ? (
                  renderTree()
                ) : (
