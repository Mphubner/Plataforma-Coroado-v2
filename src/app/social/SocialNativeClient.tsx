...
import { Heart, Users, HandHeart, ChevronRight, Instagram, Plus, HeartPulse, HeartHandshake } from 'lucide-react';
...
function PublicPortal({ userData, isLoggedIn, onLoginClick }: { userData: any, isLoggedIn: boolean, onLoginClick: () => void }) {
  ...
  React.useEffect(() => {
    const fetchPublicProfs = async () => {
      try {
        const data = await trpc.social.getPublicProfessionals.query();
        setPublicProfs(data as SocialProfessional[]);
      } catch (e) {
        console.error(e);
      } finally {
        setIsLoading(false);
  ...
  const handleBooking = async () => {
    ...
    try {
      await trpc.social.createAppointment.mutate({
            ...
            <div className="flex flex-wrap gap-4 pt-4">
              ...
              <a 
                ...
                className={cn(
                  ...
                  "border-white/10 hover:bg-white/5 rounded-full px-10 h-14 font-bold backdrop-blur-sm transition-all hover:scale-105 active:scale-95 text-white"
                )}
              >
      ...
      <section className="grid grid-cols-1 md:grid-cols-3 gap-8">
        {[
          ...
          <motion.div
            key={i}
            whileHover={{ y: -5 }}
            className="bg-zinc-900 border border-white/10 p-10 rounded-[2.5rem] flex flex-col items-center text-center space-y-4"
          >
      ...
      <section className="space-y-10">
        ...
        {isLoading ? (
          <div className="animate-pulse flex gap-4">
             <div className="bg-white/10 h-64 w-full rounded-2xl"></div>
             <div className="bg-white/10 h-64 w-full rounded-2xl"></div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {publicProfs.map((prof, index) => (
              <motion.div
                ...
                whileHover={{ y: -10 }}
              >
                <div className="bg-zinc-900 border border-white/10 rounded-[2.5rem] overflow-hidden group relative aspect-[4/5]">
              ...
              {showSuccess ? (
                ...
                <>
                  ...
                  <div className="p-6 bg-black/40 border-t border-white/10 flex gap-4">
                    ...
                    >
                      {isSubmitting ? 'Aguarde...' : selectedProf.price ? `Agendar R$ ${selectedProf.price.toFixed(2)}` : 'Confirmar'}
                    </Button>
                  </div>
                </>
              )}
...
function ProfessionalsTab() {
  ...
  const fetchProfessionals = async () => {
    setIsLoading(true);
    try {
      const data = await trpc.social.getAllProfessionals.query();
      setProfessionals(data as SocialProfessional[]);
    } catch (error) {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      await trpc.social.saveProfessional.mutate({
        id: editingProf.id,
        name: editingProf.name,
        specialty: editingProf.specialty,
        email: editingProf.email || "",
        photoUrl: editingProf.photoUrl || "",
        isPublic: editingProf.isPublic || false,
        price: editingProf.price,
        availableTimes: editingProf.availableTimes,
      });
      setShowModal(false);
      setEditingProf(null);
      fetchProfessionals();
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await trpc.social.deleteProfessional.mutate({ id });
      fetchProfessionals();
    } catch (error) {
    ...
    <div className="space-y-8 mt-8">
      <div className="flex flex-col md:flex-row gap-4 items-start md:items-center justify-between">
        <div>
          ...
          <p className="text-white/50">Gerencie a rede de profissionais.</p>
        </div>
            ...
            >
              ...
              <form onSubmit={handleSave} className="space-y-4">
                <div className="space-y-2">
                  ...
                  <Input required value={editingProf.name} onChange={e => setEditingProf({ ...editingProf, name: e.target.value })} className="bg-black border-white/10 text-white" />
                </div>
                <div className="space-y-2">
                  <label className="text-xs font-bold text-white/60">Especialidade / Cargo (ex: Psicólogo, Advogado)</label>
                  <Input required value={editingProf.specialty} onChange={e => setEditingProf({ ...editingProf, specialty: e.target.value })} className="bg-black border-white/10 text-white" />
                </div>
                <div className="space-y-2">
                  <label className="text-xs font-bold text-white/60">Email (para integração de agenda)</label>
                  <Input type="email" required value={editingProf.email} onChange={e => setEditingProf({ ...editingProf, email: e.target.value })} className="bg-black border-white/10 text-white" />
                </div>
                <div className="space-y-2">
                  <label className="text-xs font-bold text-white/60">URL Fotografia</label>
                  <Input value={editingProf.photoUrl} onChange={e => setEditingProf({ ...editingProf, photoUrl: e.target.value })} className="bg-black border-white/10 text-white" placeholder="https://..." />
                </div>
                <div className="flex gap-4">
                  <div className="space-y-2 flex-1">
                    <label className="text-xs font-bold text-white/60">Preço do Atendimento (R$)</label>
                    <Input type="number" min="0" step="0.01" value={editingProf.price || ''} onChange={e => setEditingProf({ ...editingProf, price: e.target.value ? parseFloat(e.target.value) : null })} className="bg-black border-white/10 text-white" placeholder="Ex: 50.00" />
                  </div>
                  <div className="space-y-2 flex-1">
                    <label className="text-xs font-bold text-white/60">Horários (separados por vírgula)</label>
                    <Input value={editingProf.availableTimes?.join(', ') || ''} onChange={e => setEditingProf({ ...editingProf, availableTimes: e.target.value.split(',').map(t => t.trim()) })} className="bg-black border-white/10 text-white" placeholder="14:00, 15:00" />
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2 pt-2 pb-4">
                  ...
                  <label htmlFor="isPublic" className="text-sm font-bold text-white">Mostrar publicamente</label>
                </div>
...
function AppointmentsTab() {
  ...
  const fetchAppointments = async () => {
    setIsLoading(true);
    try {
      const data = await trpc.social.getAppointments.query();
  ...
  const handleUpdateStatus = async (app: any, status: string) => {
    try {
      await trpc.social.updateAppointmentStatus.mutate({ id: app.id, status });
      fetchAppointments();
    } catch (e) {
      console.error(e);
...
export function SocialNativeClient() {
  ...
  const isAdmin = userData?.roles?.includes('admin') || userData?.roles?.includes('pastor') || userData?.roles?.includes('manager');
  ...
  if (!isAdmin) {
    return (
      <motion.div {...pagePreset} className="container mx-auto px-4 max-w-6xl">
        <PublicPortal onLoginClick={handleLoginClick} isLoggedIn={isLoggedIn} userData={userData} />
      </motion.div>
    );
  }
